...
export default function MessagesPage() {
    ...
    return (
        <div className="flex flex-col h-full bg-white transition-colors">
            ...
            <div className="flex-1 overflow-y-auto pb-20 transition-colors">
                        ...
                        >
                            {filteredConversations.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-20 px-8 text-center">
                                    <div className="w-16 h-16 bg-hover-bg rounded-full flex items-center justify-center text-text-muted mb-4 border border-border-main/50 shadow-sm transition-colors">
                                        <MessageSquare size={32} strokeWidth={1.5} />
                                    </div>
                                    <h3 className="text-sm font-bold text-text-main transition-colors">No messages yet</h3>
                                ...
                                filteredConversations.map((conv, idx) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className="w-full flex items-center gap-4 p-4 hover:bg-hover-bg border-b border-border-main/50 transition-colors text-left group relative"
                                        >
                                            <div className="relative shrink-0">
                                                <div className="w-10 h-10 rounded-full overflow-hidden bg-hover-bg border border-border-main/50 shadow-sm transition-colors">
                                                    {otherInfo.photoURL ? (
                                            ...
                                            <div className="flex-1 min-w-0 space-y-0.5">
                                                <div className="flex items-center justify-between">
                                                    <h4 className={`text-[13px] font-bold tracking-tight truncate transition-colors ${unread > 0 ? 'text-text-main' : 'text-text-main/80'}`}>
                                                ...
                                                <div className="flex items-center justify-between">
                                                    <p className={`text-xs truncate font-medium transition-colors ${unread > 0 ? 'text-text-main font-bold' : 'text-text-muted'}`}>
                                                        {lastMsg ? (
                                                            <>
                        ...
                        >
                            {filteredRequests.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-20 px-8 text-center text-text-muted">
                                    <div className="w-16 h-16 bg-hover-bg rounded-full flex items-center justify-center mb-4 border border-border-main/50 shadow-sm transition-colors">
                                        <Clock size={32} strokeWidth={1.5} />
                                    </div>
                                    <h3 className="text-sm font-bold text-text-main transition-colors">No pending requests</h3>
                                ...
                                filteredRequests.map((req, idx) => (
                                    <button
                                        ...
                                        className="w-full flex items-start gap-4 p-4 hover:bg-hover-bg border-b border-border-main/50 transition-colors text-left group"
                                    >
                                        <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center shrink-0 border border-primary/20 text-primary transition-colors">
                                            <Handshake size={20} />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between mb-1">
                                                <h4 className="text-sm font-bold text-text-main truncate transition-colors">{req.senderName}</h4>
                                            ...
                                            <p className="text-xs font-bold text-text-main leading-tight mb-1 transition-colors">Wants to learn {req.learnSkill}</p>
